...
impl<'a> Individual<'a> {
    ...
    pub fn new<R: Rng>(problem: &'a Problem, dimension: usize, rng: &mut R) -> Self {
        let x: Vec<_> = (0..dimension)
                            .map(|_| problem.domain_dist().ind_sample(rng))
                            .collect();
...
impl<'a> PartialEq<Individual<'a>> for Individual<'a> {
